...
| **Verified-facts enforcement** — no hallucinated claims about anyone | Every claim about a real party — the customer (`self`) **or a named competitor** — must match an active `verified_fact` of the same subject, or the asset is rejected ([validate.py](services/pipeline/app/pipeline/execution/validate.py)). A self fact cannot back a competitor claim. Inventing a rival's pricing on a customer's page is a factual/legal risk, so it's checked identically. |
